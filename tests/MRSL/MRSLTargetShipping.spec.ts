...
test("MRSL Target Shipping API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "targetshipping" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL Target Shipping API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);

...
test("MRSL Wegmans API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "wegmans" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL Wegmans API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);

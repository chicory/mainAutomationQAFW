...
test("MRSL Rite-Aid Pharmacy API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "rite-aid" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL Rite-Aid Pharmacy API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);

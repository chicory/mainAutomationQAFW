...
test("MRSL Amazon API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "amazon" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL Amazon API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);

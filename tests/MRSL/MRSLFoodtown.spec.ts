...
test("MRSL Foodtown API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "foodtown" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL Foodtown API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);

...
test("MRSL Stop and Shop API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "stopandshop" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL Stop and Shop API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);

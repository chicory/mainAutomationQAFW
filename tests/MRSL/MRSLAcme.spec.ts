...
test("MRSL Acme Markets API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "acmemarkets" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL Acme  Markets API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);

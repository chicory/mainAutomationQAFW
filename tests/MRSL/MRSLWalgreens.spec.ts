...
test("MRSL Walgreens Pharmacy API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "walgreens" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL Walgreens Pharmacy API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);

...
test("MRSL Shoprite API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "shoprite" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL Shoprite API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);

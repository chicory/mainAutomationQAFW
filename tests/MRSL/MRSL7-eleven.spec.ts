...
test("MRSL 7-Eleven API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "7-eleven" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL 7-Eleven API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);

...
test("MRSL Wholefoods API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "wholefoods" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL Wholefoods API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);

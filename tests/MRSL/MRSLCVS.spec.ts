...
test("MRSL CVS Pharmacy API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "cvs" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL CVS Pharmacy API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);

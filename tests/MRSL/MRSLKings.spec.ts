...
test("MRSL Kings API/Requests test", async ({
  page,
  baseUrl,
  apiEndpoint,
  zipCode,
  ...
  await page
    ...
    .contentFrame()
    .getByRole("img", { name: "kingsfoodmarkets_direct" })
    .click();
  const page1 = await page1Promise;
});

test("MRSL Kings API/Cart test", async ({ page, apiEndpointCart, baseUrl}) => {
  await page.goto(baseUrl);
